...
export default function Warehouse() {
  ...
  const [activeTab, setActiveTab] = useState<
    "inventory" | "movements" | "zones" | "analytics"
  >("inventory");
  ...
  const lowStockAlerts = warehouseItems.filter(
    (item) => item.quantity <= item.minStock || item.status === "low-stock"
  );
  ...
  const handleAddItem = () => {
    const newItemData: WarehouseItem = {
      ...
      status:
        parseInt(newItem.quantity) <= parseInt(newItem.minStock)
          ? "low-stock"
          : "in-stock",
      condition: "excellent",
    ...
    alert(
      "Item added successfully! In production, this would update the database."
    );
  ...
  const handleStockMovement = () => {
    ...
    alert(
      "Stock movement recorded successfully! In production, this would update the database."
    );
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-gray-600">
            Track inventory, manage locations, and monitor stock movements
          </p>
        </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {filteredItems.map((item) => (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between">
                        <span
                          className={`px-2 py-1 rounded-full text-xs ${getConditionColor(
                            item.condition
                          )}`}
                        >
                          {item.condition}
                ...
                <div key={zone.id} className="bg-gray-50 rounded-lg border p-4">
                  ...
                  <div className="mb-4">
                    <div className="flex items-center justify-between text-sm mb-1">
                      ...
                      <span className="font-medium">
                        {Math.round((zone.usedCapacity / zone.capacity) * 100)}%
                      </span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div
                        className={`h-2 rounded-full ${
                          zone.usedCapacity / zone.capacity > 0.9
                            ? "bg-red-500"
                            : zone.usedCapacity / zone.capacity > 0.7
                            ? "bg-yellow-500"
                            : "bg-green-500"
                        }`}
                        style={{
                          width: `${
                            (zone.usedCapacity / zone.capacity) * 100
                          }%`,
                        }}
                      />
                    </div>
                    <p className="text-xs text-gray-500 mt-1">
                      {zone.usedCapacity.toLocaleString()} /{" "}
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    value={newItem.zone}
                    onChange={(e) =>
                      setNewItem({ ...newItem, zone: e.target.value })
                    }
                    className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
                  >
                    {zones
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      value={newItem.rack}
                      onChange={(e) =>
                        setNewItem({ ...newItem, rack: e.target.value })
                      }
                      className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
                      placeholder="e.g., A1"
                    />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={newItem.shelf}
                      onChange={(e) =>
                        setNewItem({ ...newItem, shelf: e.target.value })
                      }
                      className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
                      placeholder="e.g., 01"
                    />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewItem({ ...newItem, humidity: e.target.value })
                    }
                    className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
                    placeholder="e.g., 85%"
                  />
                ...
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-yellow-800 font-medium">
                      ...
                      {lowStockAlerts.length > 1 ? "s" : ""} require
                      {lowStockAlerts.length > 1 ? "" : "s"} immediate attention
                    </p>
                  </div>
                </div>

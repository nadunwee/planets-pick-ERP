...
export default function Finance() {
  ...
  useEffect(() => {
    if (
      !currentUser ||
      (currentUser.level !== "L3" && currentUser.level !== "L4")
    ) {
      alert(
        "Access Denied: Only Finance Managers and Administrators can access Finance"
      );
      navigate("/dashboard");
    }
  ...
  const downloadPDFReport = (
    ...
    if (reportType === "trialBalance") {
      ...
      doc.setFontSize(18);
      doc.text("Planets Pick ERP System", 14, 18);
      doc.setFontSize(14);
      doc.text("Trial Balance", 14, 28);
      doc.setFontSize(10);
      doc.text(`Generated: ${new Date().toLocaleString()}`, 14, 36);
      doc.text(`As of: ${new Date().toLocaleDateString()}`, 14, 42);

      const trialBalanceRows = generateTrialBalance.map((entry) => [
        entry.accountName,
      ...
      const totalDebits = generateTrialBalance.reduce(
        (sum, entry) => sum + entry.debitBalance,
        0
      );
      const totalCredits = generateTrialBalance.reduce(
      ...
      autoTable(doc, {
        ...
        body: trialBalanceRows,
        foot: [totalsRow],
        startY: 52,
        styles: { fontSize: 9, cellPadding: 3 },
        headStyles: { fillColor: [32, 55, 117], textColor: 255 },
        footStyles: {
        ...
        columnStyles: {
          0: { halign: "left", minCellWidth: 80 },
          1: { halign: "center", minCellWidth: 40 },
          2: { halign: "right", minCellWidth: 40 },
          3: { halign: "right", minCellWidth: 40 },
        },
      ...
      const statusY = (tableMeta?.finalY ?? 52) + 10;
      ...
      const footerY = (!isBalanced ? statusY + 18 : statusY + 10) + 2;
      ...
      doc.text(
        "Generated by Planets Pick ERP System",
        14,
        Math.min(footerY, pageHeight - 10)
      );
